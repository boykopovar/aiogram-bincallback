...
def test_signed_passed_for_bool_field_raises_signed_not_applicable_error():
    class Model(BaseModel):
        flag: bool = planning_bfield(signed=False)

    with pytest.raises(SignedNotApplicableError) as excinfo:
        build_codec_plan(Model)

    assert "flag" in str(excinfo.value)


def test_signed_passed_for_enum_field_raises_signed_not_applicable_error():
    class Model(BaseModel):
...
def test_signed_passed_for_nested_basemodel_raises_signed_not_applicable_error():
    class Inner(BaseModel):
