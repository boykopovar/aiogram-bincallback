...
- поле-модель не принимает `bits`, `item_bits`, `bin_order`, `max_len`, `signed` - эти параметры относятся к полям внутри вложенной модели, передача любого из них - `UnrecognizedBinFieldParamError`;
